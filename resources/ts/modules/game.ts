import { renderWords, mainScreen, statsScreen, wordsAnimation, updateTimerDisplay, updateWordsProgress, showTimer, hideTimer, moveCursor, enterFocusMode, exitFocusMode, isFocusModeActive } from "./ui";
import { calculateMetrics, resetStats, setStartTime } from "./stats";
...
export async function newGame() {
  ...
  if (isWordsMode) {
    ...
    const activeCount = activeBtn?.textContent ? parseInt(activeBtn.textContent, 10) : NaN;
    const wordCount = Number.isFinite(activeCount) ? activeCount : currentWordsCount;
  ...
  for (const word of wordSpan.children) {
    ...
    word.setAttribute("size", String(word.children.length));
